# invoicing — Cebelca.biz skills

Two skills that drive the
...
- `curl` and `python3` (standard library only, no packages)
...
```bash
./install.sh invoice invoices-download
```

This copies both skills into `~/.claude/skills/`. Then `invoice`'s `setup.sh`
asks for your API token (input is hidden, saved to
`~/.claude/skills/invoice/.token` with mode 600). It never overwrites a token or
config you already have, so re-run it any time to update the skills.
`invoices-download` calls `invoice`'s `cebelca.sh`, so install both.
...
folders (without `setup.sh`) into `~/.claude/skills/`, then write the token to
...
| Symptom | Cause |
|---------|-------|
| `ERROR: no API token` | No `.token` file; run `./install.sh invoice` or create it by hand |
...
```
invoice/              SKILL.md · cebelca.sh · config.example.sh · setup.sh · README.md
invoices-download/    SKILL.md · download.sh · README.md
```

`invoice/setup.sh` runs at install time (token prompt) and is not installed.
Your token and `config.sh` stay out of git (see the repo's `.gitignore`).
